...
Mesh.prototype.startScuttling = function()
{
	...
	if (this.options.server)
	{
		...
		{
			self.logger.info('scuttler bound to ' + self.options.host + ':' + self.options.gossipport);
			deferred.resolve('OK');
		});
...
Mesh.prototype.handleJoiningNode = function(node)
{
	if (node.id === this.id)
		return;
	this.logger.info('node joining', node);
...
{
	// IMPLEMENT
	this.logger.info('node changing', node);
};
...
Mesh.prototype.handleLeavingNode = function(node)
{
	if (node.id === this.id)
		return;
	this.logger.info('node leaving', node);
