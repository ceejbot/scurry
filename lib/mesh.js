...
Mesh.prototype.connect = function()
{
	...
	.then(function()
	{
		self.restify.listen(self.options.port, self.options.host, function()
		{
			console.log('    restify server bound to ' + self.options.host + ':' + self.options.port);
			deferred.resolve('OK');
		});
	})
	.fail(function(err)
...
Mesh.prototype.startScuttling = function()
{
	...
	if (this.options.server)
	{
		...
		this.server.listen(this.options.gossipport, this.options.host, function()
		{
			console.log('    scuttler bound to ' + self.options.host + ':' + self.options.gossipport);
			deferred.resolve('OK');
		});
	}
	else
	{
		console.log('   scuttler looking for network at ' + self.options.host + ':' + self.options.gossipport);
		this.connection = net.connect(this.options.gossipport, this.options.host, this.scuttler.bind(this));
	}
...
{
	stream
		.pipe(this.meshdata.createStream())
		.pipe(stream);
