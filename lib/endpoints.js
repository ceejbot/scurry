...
function proxyRequest(remote, request, response, next)
{
	...
	var relay = http.request(options, relayResponse =>
	{
...
function handleGetBucket(request, response, next)
{
	...
	_.each(Object.keys(nodehash), k =>
	{
		var node = nodehash[k];
		if (node.isremote)
		{
			makeRequest(node, uri, 'GET', rez =>
			{
				keystreams.push(rez);
				count--;
				if (count === 0)
...
function handleGetItem(request, response, next)
{
	...
	node.get(bucket, id).then(value =>
	{
		if (value)
		{
			...
			response.send(200, Buffer.from(value.payload, 'base64').toString());
		}
		else
			response.send(404);
		next();
