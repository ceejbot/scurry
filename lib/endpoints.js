...
function handleGetBucket(request, response, next)
{
	...
	var nodes = _.map(Object.keys(nodehash), function(k) { return nodehash[k]; });
	...
	_.each(nodes, function(node)
	{
		if (!node.isremote)
		{
			// local node, just get our keystream
			keystreams.push(mesh.mynode.keys(bucket));
		}
...
function handleGetKeyStream(request, response, next)
{
	...
	request.log.info('GET keystream for ' + bucket);
